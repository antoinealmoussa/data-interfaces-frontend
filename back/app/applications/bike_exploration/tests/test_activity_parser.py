...
def _fit_side_effect(content: bytes, filename: str) -> list[tuple[float, float]]:
...
class TestParseZipArchive:
    ...
    def test_filters_cycling_only(self, mock_parse_fit: object, sample_zip_bytes: bytes) -> None:
        activities = parse_zip_archive(sample_zip_bytes, "export.zip")
    ...
    def test_empty_zip_returns_empty(self) -> None:
        ...
        assert parse_zip_archive(buf.getvalue(), "empty.zip") == []
