...
const mockItems = [
  {
    label: "Gestion",
    path: "management",
    icon: <Box data-testid="icon-management" />,
  },
  {
    label: "Tournoi",
    path: "tournament",
    icon: <Box data-testid="icon-tournament" />,
  },
];
...
const renderSidebar = (
  props: Partial<Parameters<typeof GenericSidebar>[0]> = {},
) => {
  return render(
    <MemoryRouter initialEntries={["/rugby-teams"]}>
      <GenericSidebar
        ...
        selectedTeamName={props.selectedTeamName ?? "Équipe A"}
...
describe("GenericSidebar", () => {
  ...
  it("devrait contenir un sélecteur d'équipe", () => {
    ...
    const equipeLabel = Array.from(labels).find(
      (l) => l.textContent === "Équipe",
    );
  ...
  it("devrait contenir un sélecteur de saison", () => {
    const { container } = renderSidebar();
    const labels = container.querySelectorAll("label");
    const saisonLabel = Array.from(labels).find(
      (l) => l.textContent === "Saison",
    );
